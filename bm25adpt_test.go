...
func TestNewBM25Adpt(t *testing.T) {
	...
	_, err := NewBM25Adpt(corpus, tokenizer, -1.0, 0.75, 1.0, nil)
	if err == nil {
	...
	_, err = NewBM25Adpt(corpus, tokenizer, 1.2, 1.5, 1.0, nil)
	if err == nil {
	...
	_, err = NewBM25Adpt(corpus, tokenizer, 1.2, 0.75, -1.0, nil)
	if err == nil {
...
func TestBM25AdptGetScores(t *testing.T) {
	corpus := []string{"hello world", "this is a test"}
	tokenizer := func(s string) []string { return strings.Split(s, " ") }
	bm25, _ := NewBM25Adpt(corpus, tokenizer, 1.2, 0.75, 1.0, nil)
