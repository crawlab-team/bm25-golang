...
func TestCountTermFreq(t *testing.T) {
	...
	freq, err := CountTermFreq("nonexistent", "hello world", tokenizer)
	if err != nil {
	...
	freq, err = CountTermFreq("hello", "hello world", tokenizer)
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	if freq != 1 {
...
func TestTopNIndices(t *testing.T) {
	...
	indices, err = TopNIndices([]float64{1.0, 2.0, 2.0, 3.0}, 3)
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	expected = []int{3, 1, 2}
	...
	indices, err = TopNIndices([]float64{-1.0, 2.0, -3.0, 4.0}, 2)
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	expected = []int{3, 1}
...
func TestJoinTokens(t *testing.T) {
	...
	joined = JoinTokens([]string{"hello", "world"}, "-")
	if joined != "hello-world" {
		t.Errorf("Expected 'hello-world', but got '%s'", joined)
	}

	joined = JoinTokens([]string{"hello", "world", "test"}, "")
